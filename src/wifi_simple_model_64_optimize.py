...
shop = pd.read_csv(path + 'shop_info.csv')
...
for mall in mall_list:
    ...
    feature = [x for x in train1.columns if
               x not in ['row_id', 'user_id', 'label', 'shop_id', 'time_stamp', 'mall_id', 'wifi_infos']]
    xgbtrain = xgb.DMatrix(df_train[feature], df_train['label'])
    xgbtest = xgb.DMatrix(df_test[feature])
    watchlist = [(xgbtrain, 'train'), (xgbtrain, 'test')]
    num_rounds = 60
    model = xgb.train(params, xgbtrain, num_rounds, watchlist, early_stopping_rounds=5, verbose_eval=20)
    ...
    print(accuracy_score(df_train['label'], model.predict(xgb.DMatrix(df_train[feature]))))
result.to_csv('../output/result/result.csv', index=None)
